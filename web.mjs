...
;
	($.$my_app) = class $my_app extends ($.$mol_book2) {
		Add_icon(){
		...
		Email(id){
			...
			(obj.label) = () => ([(this.email_title(id)), (this.Email_copy(id))]);
			(obj.content) = () => ([(this.email_value(id))]);
			return obj;
		}
		emails(){
		...
		Title_control(){
			...
			(obj.value) = (next) => ((this.title_new(next)));
			return obj;
		}
		Title_field(){
		...
		Email_control(){
			const obj = new this.$.$mol_string();
			(obj.value) = (next) => ((this.email_new(next)));
			return obj;
		}
		Email_field(){
			const obj = new this.$.$mol_form_field();
		...
		Form(){
			...
			(obj.form_fields) = () => ([(this.Title_field()), (this.Email_field())]);
			(obj.buttons) = () => ([(this.Email_add())]);
			return obj;
		}
		Email_add_page(){
			const obj = new this.$.$mol_page();
		...
		message(){
			return {"required": (this.$.$mol_locale.text("$my_app_message_required")), "duplicate": (this.$.$mol_locale.text("$my_app_message_duplicate"))};
		}
		pages(){
			return [(this.Email_list_page()), (this.Email_add_page())];
		}
	};
	($mol_mem(($.$my_app.prototype), "Add_icon"));
	($mol_mem(($.$my_app.prototype), "Email_new"));
	($mol_mem_key(($.$my_app.prototype), "Email_copy"));
	($mol_mem_key(($.$my_app.prototype), "Email"));
	($mol_mem(($.$my_app.prototype), "Emails"));
	($mol_mem(($.$my_app.prototype), "Email_list_page"));
	($mol_mem(($.$my_app.prototype), "Close_icon"));
	($mol_mem(($.$my_app.prototype), "Close"));
	($mol_mem(($.$my_app.prototype), "title_new"));
	($mol_mem(($.$my_app.prototype), "Title_control"));
	($mol_mem(($.$my_app.prototype), "Title_field"));
	($mol_mem(($.$my_app.prototype), "email_new"));
	($mol_mem(($.$my_app.prototype), "Email_control"));
	($mol_mem(($.$my_app.prototype), "Email_field"));
	($mol_mem(($.$my_app.prototype), "email_add"));
	($mol_mem(($.$my_app.prototype), "Email_add"));
	($mol_mem(($.$my_app.prototype), "Form"));
	($mol_mem(($.$my_app.prototype), "Email_add_page"));
...
(function ($) {
    ...
    (function ($$) {
        class $my_app extends $.$my_app {
            ...
            email_bid() {
                if (this.emails().findIndex(obj => obj.value === this.email_new().trim()) !== -1)
        ...
        __decorate([
            $mol_mem
        ], $my_app.prototype, "email_rows", null);
        __decorate([
            $mol_action
        ], $my_app.prototype, "email_add", null);
        __decorate([
            $mol_mem
        ], $my_app.prototype, "pages", null);
        $$.$my_app = $my_app;
    })($$ = $.$$ || ($.$$ = {}));
})($ || ($ = {}));
